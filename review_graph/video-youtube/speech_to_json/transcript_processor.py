...
def count_tokens(text, model="gpt-4-turbo"):
    ...
    Counts the number of tokens in a given text for a specified model:
        - Uses the tokenizer for the specified model to encode the text
        - Returns the number of tokens generated
    Inputs:
        text (str): The input text to tokenize
        model (str): The model name used for tokenization (default: "gpt-4-turbo")
    Output:
...
def transcript_to_json(raw_text):
	"""
    Converts a Vietnamese transcript into structured JSON review data using GPT:
        - Constructs a detailed prompt instructing GPT to extract product review information
        - Sends the prompt to GPT and receives a JSON-formatted response
        - Cleans the response to remove markdown formatting
        - Returns the raw JSON string or original text if an error occurs
    Inputs:
        raw_text (str): The transcript text to be processed
    Output:
        str: A cleaned JSON string containing structured product review data, or the original transcript if an error occurs.
    """
